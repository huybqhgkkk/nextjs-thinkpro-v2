...
const singleApi = async (dataApi = mandatory(), dispatch) => {
	const { url = mandatory(), options, payload = {}, beforeCallType, successType, errorType, next = f => f, records, typeApi } = dataApi;
	...
	try {
		...
		if (url != '') {
			...
			if (successType) {
				dispatch({ type: successType, payload: response, records });
			}

			return response;
		}
		if (successType) {
			dispatch({ type: successType, records });
		}
		return records;
	} catch (error) {
		next(error);
